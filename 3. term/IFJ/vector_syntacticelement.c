...
int vector_SyntacticElement_init(Vector_SyntacticElement *vector){
	return vector_SyntacticElement_init_sized(vector, VECTOR_SyntacticElement_DEFAULT_SIZE);
}
...
int vector_SyntacticElement_init_sized(Vector_SyntacticElement *vector, int size){
	...
	if((vector->array=malloc(sizeof(SyntacticElement)*size))==NULL){
		vector->size=0;
		return RET_INTERN;
	}
	else{
...
static int grow_vector(Vector_SyntacticElement *vector){
	size_t nsize;
	//if size of vector is lower than the default
	if(vector->size<VECTOR_SyntacticElement_DEFAULT_SIZE*2){
		//just set default*2
		nsize=VECTOR_SyntacticElement_DEFAULT_SIZE*2;
	}
	//otherwise double
	else{
		nsize=2*vector->size;
	...
	if((tmp=realloc(vector->array, nsize*sizeof(SyntacticElement)))==NULL){
		return RET_INTERN;
	}
	else{
		vector->size=nsize;
...
int vector_SyntacticElement_insert_at(Vector_SyntacticElement *vector, size_t index, SyntacticElement value){
	int retval=vector_SyntacticElement_push_back(vector, value);
	if(retval!=RET_EOK){
	...
	assert(vector->last>=index);
	memmove(vector->array+index+1, vector->array+index, (vector->last+1-index)*sizeof(SyntacticElement));
