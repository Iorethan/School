...
***/
	//Vzdy rusim aktualni polozku, zacinam od zacatku seznamu
	L->Act = L->First;
	//Prochazim cely seznam polozku po polozce
	while (L->First != NULL)
	{
		//Posunu zacatek na dalsi polozku
		L->First = L->Act->ptr;
		//Uvolnim aktualni polozku a posunu se na dalsi
		free(L->Act);
		L->Act = L->First;
	}
	L->Act = NULL;
...
** volá funkci Error().
**/
	//Pomocna promena pro vytvoreni nove polozky
	tElemPtr aux = malloc(sizeof(struct tElem));
	if (aux == NULL)
		Error();
	else
	{
		//Naplnim novou polozku daty
		aux->data = val;
		aux->ptr = L->First;
		//Zmenim ukazatel na zacatek seznamu
		L->First = aux;
	}	
}
...
**/
	//Pomocna promena pro mazanou polozku
	tElemPtr aux = L->First;
	//Seznam neni prazdny
	if (aux != NULL)
	{
...
** nic se nedìje.
**/
	//Seznam je aktivni a aktivni prvek ma naslednika
	if (L->Act != NULL && L->Act->ptr != NULL)
	{
		tElemPtr aux = L->Act->ptr;
		//Ukazatel v aktivnim prvku posunu za mazany prvek
		L->Act->ptr = aux->ptr;
		free(aux);
	}
}
...
**/
	if (L->Act != NULL)
	{
		...
		else
		{
			...
			aux->data = val;
			//Vlozim novy prvek za aktivni a spojim seznam
			aux->ptr = L->Act->ptr;
			L->Act->ptr = aux;
		}
	}
